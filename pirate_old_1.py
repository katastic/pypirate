...
page = requests.get("https://thepiratebay10.org/top/all")
...
#		print(uploaded_date)
	...
	#print("uploaded:", uploaded_date)
	#print("size:", size)

	t.add_row([name, seeds, leeches, proper_datetime, size, raw_size])
	
	rank=i
	abc = "INSERT INTO data3 VALUES (NULL, '{0}',{1},'{2}',{3},{4},'{5}','{6}')".format(name.replace("'",""), rank, capture_datetime, seeds, leeches, proper_datetime, raw_size)	
#	print(sql_string)	
	c.execute(abc)
#	c.execute(sql_string)
#CREATE TABLE data ( id integer PRIMARY KEY, name text NOT NULL, seeds integer not null, leaches integers not null, datetime text not null, size integer not null );

c.commit()
c.close()
print(t)
print("Results for: ", capture_datetime)
